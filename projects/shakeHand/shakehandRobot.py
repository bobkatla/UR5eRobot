...
def getPoses(h):
    subprocess.check_output(["python", "../commonSources/extractData/record.py", "--host", str(h), "--samples", "1", "--frequency", "5", "--config", "../commonSources/extractData/config/curQ_record_config.xml"])
...
subprocess.check_output(["python", "../measureForce/getForceData.py"])
a = pd.read_csv("robot_data.csv")
